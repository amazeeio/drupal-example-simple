Docker Compose Drupal 9 postgres - php7.4, nginx, postgres
==========================================================
...
docker run --rm --net drupal9-example-postgres_default jwilder/dockerize dockerize -wait tcp://postgres:5432 -timeout 1m
```
...
docker ps --filter label=com.docker.compose.project=drupal9-example-postgres | grep Up | grep drupal9-example-postgres_nginx_1
docker ps --filter label=com.docker.compose.project=drupal9-example-postgres | grep Up | grep drupal9-example-postgres_postgres_1
docker ps --filter label=com.docker.compose.project=drupal9-example-postgres | grep Up | grep drupal9-example-postgres_php_1
docker ps --filter label=com.docker.compose.project=drupal9-example-postgres | grep Up | grep drupal9-example-postgres_cli_1

# Should ssh against the cli container by default
docker-compose exec -T cli bash -c "env | grep LAGOON=" | grep cli-drupal

# Should have the correct environment set
docker-compose exec -T cli bash -c "env" | grep LAGOON_ROUTE | grep drupal9-example-postgres.docker.amazee.io
